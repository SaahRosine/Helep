...
class RoundRobinMatcher:
    ...
    def pick(self, victim_lat, victim_lon, responders):
        resp_list = list(responders)
        if not resp_list:
            return None
        RoundRobinMatcher._index = (RoundRobinMatcher._index + 1) % len(resp_list)
        return {"id": resp_list[RoundRobinMatcher._index]["id"], "strategy": "round-robin"}
